...
class ReadFunction {
  ...
  func readChunk(fileRegion: FileRegion)
  ...
  func getSize(_ fileRegion: FileRegion) -> Int {
    return fileRegion.endIndex - fileRegion.readerIndex + 1
  }

  func getChunkFileRegion(_ chunkSize: Int, _ fileRegion: FileRegion) -> FileRegion {
  ...
  func getRemainderFileRegion(_ chunkSize: Int, _ fileRegion: FileRegion) -> FileRegion {
    return FileRegion(fileHandle: fileRegion.fileHandle,
                                     readerIndex: fileRegion.readerIndex + chunkSize - 1,
                                     endIndex: fileRegion.readerIndex)
  }

  func readChunked(_ fileInfo: FileInfo)
  ...
  func read(request: Google_Bytestream_ReadRequest)
    -> EventLoopFuture<GRPCStatus> {
    ...
    let calculateFileInfo = openEvent.flatMap{
      ...
      let fileRegion = FileRegion(fileHandle: handle, readerIndex: readerIndex, endIndex: endIndex)
